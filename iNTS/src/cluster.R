...
library(ggsci)
library(factoextra)
EC <- read.csv('nation1.csv',header = T)  ## 读取我们的数据
...
country_name <- subset(EC,EC$year==2021 & 
...
###画图
p<-fviz_dend(res.hc, cex = 0.5, k = 4, color_labels_by_k = TRUE,
          k_colors=c('#3399FF','lightblue','red','#FFCC66'),
          horiz = T, rect = TRUE)
p
library(eoffice)
topptx(p,"cluster_2021_Deaths&Incidence.pptx")
